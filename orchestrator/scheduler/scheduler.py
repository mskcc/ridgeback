...
class Scheduler(object):
    ...
    def get_jobs_to_submit():
        ...
        short_jobs_count = Job.objects.filter(
            status__gte=Status.SUBMITTING, walltime__lt=settings.SHORT_JOB_MAX_DURATION
        ).count()
        ...
        long_jobs_count = Job.objects.filter(
            status__gte=Status.SUBMITTING, walltime__gte=settings.MEDIUM_JOB_MAX_DURATION
        ).count()
