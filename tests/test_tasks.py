...
class TestTasks(TestCase):
    ...
    def test_submit_polling_lsf(self, job_submitter, submit_job_to_batch_system, init):
    ...
    @patch("submitter.toil_submitter.toil_jobsubmitter.ToilJobSubmitter.__init__")
    @patch("orchestrator.tasks.submit_job_to_batch_system")
    @patch("batch_systems.slurm_client.slurm_client.SLURMClient.submit")
    @skip("Need to mock memcached lock")
    def test_submit_polling_slurm(self, job_submitter, submit_job_to_batch_system, init):
    ...
    def test_submit_lsf(self, save_job_info, submit):
        with override_settings(BATCH_SYSTEM="LSF"):
    ...
    @patch("batch_systems.slurm_client.slurm_client.SLURMClient.submit")
    @patch("orchestrator.tasks.save_job_info")
    def test_submit_slurm(self, save_job_info, submit):
        with override_settings(BATCH_SYSTEM="SLURM"):
            save_job_info.return_value = None
            submit.return_value = self.submitting_job.external_id
            submit_job_to_batch_system(self.submitting_job)
    ...
    def test_job_args_slurm(self):
        with override_settings(BATCH_SYSTEM="SLURM"):
            ...
            memlimit = None
            inputs = {}
            expected_job_group = "--wckey={}".format(job_id)
            jobsubmitterObject = ToilJobSubmitter(
                job_id, app, inputs, root_dir, resume_jobstore, walltime, tool_walltime, memlimit
            )
            job_group = " ".join(jobsubmitterObject._job_group())
    ...
    def test_job_args_walltime_lsf(self):
        with override_settings(BATCH_SYSTEM="LSF"):
            job_id = str(uuid.uuid4())
            app = {"github": {"repository": "awesome_repo", "entrypoint": "test.cwl"}}
            root_dir = "test_root"
    ...
    def test_job_args_walltime_slurm(self):
        with override_settings(BATCH_SYSTEM="SLURM"):
            job_id = str(uuid.uuid4())
            app = {"github": {"repository": "awesome_repo", "entrypoint": "test.cwl"}}
            root_dir = "test_root"
    ...
    def test_job_args_tool_walltime_lsf(self):
        with override_settings(BATCH_SYSTEM="LSF"):
            job_id = str(uuid.uuid4())
            app = {"github": {"repository": "awesome_repo", "entrypoint": "test.cwl"}}
            root_dir = "test_root"
    ...
    def test_job_args_tool_walltime_slurm(self):
        with override_settings(BATCH_SYSTEM="SLURM"):
            job_id = str(uuid.uuid4())
            app = {"github": {"repository": "awesome_repo", "entrypoint": "test.cwl"}}
            root_dir = "test_root"
            ...
            memlimit = None
            inputs = {}
            expected_tool_args = "--time={}".format(walltime_hard)
            jobsubmitterObject = ToilJobSubmitter(
                job_id, app, inputs, root_dir, resume_jobstore, walltime, tool_walltime, memlimit
            )
            tool_args_list = jobsubmitterObject._tool_args()
    ...
    def test_job_args_memlimit_slurm(self):
        with override_settings(BATCH_SYSTEM="SLURM"):
            job_id = str(uuid.uuid4())
            app = {"github": {"repository": "awesome_repo", "entrypoint": "test.cwl"}}
            root_dir = "test_root"
    ...
    def test_job_args_all_options_lsf(self):
        with override_settings(BATCH_SYSTEM="LSF"):
            job_id = str(uuid.uuid4())
            app = {"github": {"repository": "awesome_repo", "entrypoint": "test.cwl"}}
            root_dir = "test_root"
    ...
    def test_job_args_all_options_slurm(self):
        with override_settings(BATCH_SYSTEM="SLURM"):
            job_id = str(uuid.uuid4())
            app = {"github": {"repository": "awesome_repo", "entrypoint": "test.cwl"}}
            root_dir = "test_root"
    ...
    @patch("orchestrator.tasks.get_job_info_path")
    @patch("batch_systems.lsf_client.lsf_client.LSFClient.status")
    def test_fail_lsf(self, status, get_job_info_path, command_processor):
        with override_settings(BATCH_SYSTEM="LSF"):
            self.current_job.status = Status.PENDING
            self.current_job.save()
            command_processor.return_value = True
    ...
    @patch("orchestrator.tasks.get_job_info_path")
    @patch("batch_systems.slurm_client.slurm_client.SLURMClient.status")
    def test_fail_slurm(self, status, get_job_info_path, command_processor):
        with override_settings(BATCH_SYSTEM="SLURM"):
            self.current_job.status = Status.PENDING
            self.current_job.save()
            command_processor.return_value = True
    ...
    @patch("orchestrator.tasks.get_job_info_path")
    @patch("batch_systems.lsf_client.lsf_client.LSFClient.status")
    def test_running_lsf(self, status, get_job_info_path, command_processor):
        with override_settings(BATCH_SYSTEM="LSF"):
            self.current_job.status = Status.PENDING
            self.current_job.save()
            command_processor.return_value = True
    ...
    @patch("orchestrator.tasks.get_job_info_path")
    @patch("batch_systems.slurm_client.slurm_client.SLURMClient.status")
    def test_running_slurm(self, status, get_job_info_path, command_processor):
        with override_settings(BATCH_SYSTEM="SLURM"):
            self.current_job.status = Status.PENDING
            self.current_job.save()
            command_processor.return_value = True
    ...
    def test_fail_not_submitted_lsf(self, status, command_processor):
        with override_settings(BATCH_SYSTEM="LSF"):
            command_processor.return_value = True
    ...
    def test_cleanup_lsf(self, cleanup_folders):
        with override_settings(BATCH_SYSTEM="LSF"):
            Job.objects.create(
    ...
    @patch("orchestrator.tasks.cleanup_folders")
    def test_cleanup_slurm(self, cleanup_folders):
        with override_settings(BATCH_SYSTEM="SLURM"):
            Job.objects.create(
                type=PipelineType.CWL,
                app={"app": "link"},
            ...
            calls = [
                call(str(job_old_completed.id), exclude=["input.json", "slurm.log"]),
                call(str(job_old_failed.id), exclude=["input.json", "slurm.log"]),
            ]
